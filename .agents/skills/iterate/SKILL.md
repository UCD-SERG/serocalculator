---
name: iterate
description: Drive a pull request to a clean review verdict by looping request-review → address every finding → re-request-review until there are zero flagged items. Use when asked to "iterate until clean", "address the review comments", "@Codex review again and fix what it finds", or after opening a PR you want carried all the way to mergeable. Handles the @Codex bot reviewer and human reviewers.
user-invocable: true
allowed-tools:
  - Bash
  - Read
...
- "iterate until clean", "address the review comments", "keep going until the
  reviewer's happy".
- "@Codex review again" / iterative review loops on a PR.
...
1. **Claim the PR** (first round only). Post a brief comment so a parallel
   `@Codex` CI run or another person doesn't start a colliding session:
   `gh pr comment <N> --body "Codex CLI (local session) is working on
...
3. **Request the review.**
   - `@Codex` bot reviewer: post `@Codex review` on the PR (or the repo's
     equivalent trigger).
...
4. **Wait for the review to land, then read the LATEST one.** Don't trust an
   ...
   gh pr view <N> --json comments \
     --jq '[.comments[] | select(.author.login | startswith("Codex"))] | last | .body'
   ```
   The reviewer's bot login varies by API and setup: `gh pr view` reports it
   as `Codex`, the REST API (`gh api .../comments`) as `Codex[bot]`, and
   some setups post reviews as `github-actions[bot]`. `startswith("Codex")`
   matches the @Codex bot across both `gh pr view` and `gh api` — broaden it
