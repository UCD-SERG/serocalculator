...
* Render documentation vignettes as HTML only for the altdoc website (removed leftover `revealjs`/`pptx` output formats from `vignettes/_metadata.yml` and `simulate_xsectionalData.qmd`), fixing a quarto output-filename collision that broke the docs build. (#503)
