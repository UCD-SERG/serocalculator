analyze_sims <- function(data, true_lambda) {
  ...
  true_value <- mean(data$incidence.rate, na.rm = TRUE)
  bias <- mean(data$incidence.rate - true_value, na.rm = TRUE)

  # Standard Error (Mean of reported standard errors)
  standard_error <- mean(data$SE, na.rm = TRUE)

  # RMSE (Root Mean Square Error)
  rmse <- sqrt(mean((data$incidence.rate - true_value)^2, na.rm = TRUE))
