...
#'
#' @description
#' Function to estimate seroincidences based
#' on cross-section serology data and longitudinal
#' response model.
#'
#' @param pop_data a [data.frame] with cross-sectional
#' serology data per antibody and age, and additional
...
#' calculations when computing by strata. If set to more
#' than 1 and package \pkg{parallel} is available, then
...
#' i.e., a list of `"seroincidence"` objects
#' from [est.incidence()], one for each stratum,
...
est.incidence.by <- function(
  ...
  warning(
    ...
    "consider using the `est.incidence()`
    function to simplify your code and avoid this warning.",
