...
#' @param lambda.start starting value for incidence rate
...
#' @inheritDotParams stats::nlm -f -p -hessian -print.level
#'
    ...
    ...)
{
  ...
  if(verbose)
  {
    message("nrow(curve_params) = ",
            nrow(curve_params |> dplyr::filter(.data[["antigen_iso"]] %in% .env[["antigen_isos"]])))
  }
  ps = list()
  cs = list()
  conds = list()
