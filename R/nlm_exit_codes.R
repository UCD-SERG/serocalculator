nlm_exit_codes <- c(
  ...
  "3" = "3: Last global step failed to locate a point lower than x. Either x is an approximate local minimum of the function, the function is too non-linear for this algorithm, or `stepmin` in `estimate_scr()` (a.k.a. `steptol` in `nlm()`) is too large.",
