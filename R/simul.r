...
baseline <- function(kab,yvec){
  subst <- which(yvec < blims[kab,2]); k <- 1;
  while(k <= length(subst)){
    yvec[subst[k]] <- mkbaseline(kab,1);
    k <- k + 1;
  }
  return(yvec);
}
...
simresp.tinf <- function(lambda,t.end,age.fx,ablist,n.mc=0,renew.params){
  mcsize <- dim(predpar)[3]; nmc <- n.mc;
  if(n.mc==0) nmc <- sample.int(n=mcsize,size=1);
  n.ab <- length(ablist);
  t0 <- 0; t <- c(); b <- c(); y <- c(); t.step <- 1; smp <- c(); t.inf <- c();
  #set.seed(975313579)
  t.next <- -log(runif(1, 0, 1))/lambda # time to first infection...
  if(!is.na(age.fx)) mcpar <- ldpar(age.fx,ablist,nmc);
  if(is.na(age.fx)) mcpar <- ldpar(t.next/day2yr,ablist,nmc);
  par.now <- mcpar;
  b.inf <- mcpar[2,]; # b0
  b.end <- rep(0,n.ab);
  if(t.next > t.end) t.next <- t.end - t0;
  if(t.next < t.end) t.inf <- c(t.inf,t.next);
  t.now <- seq(from=0, to=t.next, by=t.step)
  b.now <- array(0,dim=c(length(t.now),n.ab));
  y.now <- array(0,dim=c(length(t.now),n.ab));
  for(k.ab in 1:n.ab) y.now[,k.ab] <- mkbaseline(k.ab,length(t.now));
  t <- c(t,t0 + t.now);
  b <- rbind(b,b.now); y <- rbind(y,y.now);
  y.end <- as.matrix(y.now)[nrow(y.now),];
  if(n.ab==1 && y.end==0) y.end <- par.now[1];
  if(n.ab > 1) y.end[y.end==0] <- par.now[1,y.end==0];
  while(t0 < t.end - t.next){
    t0 <- t0 + t.next;
    if(!renew.params){
      if(!is.na(age.fx)) par.now <- ldpar(age.fx,ablist,nmc);
      if(is.na(age.fx)) par.now <- ldpar(t0/day2yr,ablist,nmc);
      b0 <- b.inf; # b0 <- runif(n=1,min=1,max=200); not implemented
      par.now[1,] <- y.end; # y0 = y at end of prior episode
    }
    t.next <- -log(runif(1, 0, 1))/lambda
    if(t0 <= t.end & t0 + t.next > t.end) t.next <- t.end - t0;
    if(t0 + t.next < t.end) t.inf <- c(t.inf,t0 + t.next);
    ...
    t <- c(t,t0 + t.now);
    b <- rbind(b,b.now); y <- rbind(y,y.now);
    b.end <- b[nrow(as.matrix(b)),]; y.end <- y[nrow(as.matrix(y)),];
    if(renew.params)
      if(n.mc==0) nmc <- sample.int(n=mcsize,size=1);
      if(!is.na(age.fx)) par.now <- ldpar(age.fx,ablist,nmc);
      if(is.na(age.fx)) par.now <- ldpar((t0 + t.next)/day2yr,ablist,nmc);
  }
  return(list(t=t,b=b,y=y,smp=smp,t.inf=t.inf));
}
...
simcs.tinf <- function(lambda,n.smpl,age.rng,age.fx=NA,
                       ablist,n.mc=0,renew.params=FALSE){
  st.days <- round(age.rng[1]); # from min=age.rng[1] days...
  en.days <- round(age.rng[2]); # to   max=age.rng[2] days...
  if(st.days==0) st.days <- 1;
  # if(en.days>30000) en.days <- 30000;
  y.smpl <- array(NA,dim=c(n.smpl,length(ablist)+1)); # y and age
  for(k.smpl in 1:n.smpl){
    resp <- simresp.tinf(lambda,t.end=en.days,age.fx=age.fx,ablist=ablist,
                         n.mc=n.mc,renew.params=renew.params);
    tinf.smp <- sample((st.days:en.days),size=1);# sample at random age
    y.smpl[k.smpl,] <- c(resp$t[tinf.smp],as.matrix(resp$y)[tinf.smp,]);
  }
  return(y.smpl);
}
