...
#'
#' @param object a `summary.seroincidence.by` object (generated by applying the `summary()` method to the output of [estimate_scr_by()]).
#' @param yvar the name of a stratifying variable in `object`.
#' @param color_var [character] the name of the variable used for fill color (e.g., "Country").
...
strat_ests_barplot <- function(
  ...
  if (CIs && "incidence.rate.CI.lower" %in% names(object) && "incidence.rate.CI.upper" %in% names(object)) {
    plot1 <- plot1 +
      ggplot2::geom_errorbar(ggplot2::aes(
        xmin = .data$incidence.rate.CI.lower * 1000,
        xmax = .data$incidence.rate.CI.upper * 1000
