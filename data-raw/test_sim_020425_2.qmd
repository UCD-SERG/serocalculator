...
compute_statistics <- function(data, true_lambda) {
...
statistics_50<-compute_statistics(final_table_50, true_lambda)
statistics_100<-compute_statistics(final_table_100, true_lambda)
statistics_150<-compute_statistics(final_table_150, true_lambda)
statistics_200<-compute_statistics(final_table_200, true_lambda)
...
simulate_seroincidence2 <- function(nrep, n_sim) {
  ...
  results <- future_map(1:n_sim, function(i) {
    ...
    csdata <- sim.cs(
      ...
      antigen_isos = antibodies,
      n.mc = 0,
      renew_params = FALSE,  # Use different parameters for each simulation
      add.noise = TRUE,
      noise_limits = dlims,
      format = "long"
    )
...
```{r, echo=FALSE, message=FALSE}
# Store each sample size's summary as table
final_table_50_2 <- generate_final_table(results_50_2, 50)
final_table_100_2 <- generate_final_table(results_100_2, 100)
final_table_150_2 <- generate_final_table(results_150_2, 100)
final_table_200_2 <- generate_final_table(results_200_2, 100)
...
```{r}
statistics_50_2<-compute_statistics(final_table_50_2, true_lambda)
statistics_100_2<-compute_statistics(final_table_100_2, true_lambda)
statistics_150_2<-compute_statistics(final_table_150_2, true_lambda)
statistics_200_2<-compute_statistics(final_table_200_2, true_lambda)
