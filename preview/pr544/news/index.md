...
- Fixed multi-way cluster-robust standard error calculation in
  ...
  `cluster_var = c("commune", "household_id")` now uses a true multi-way
  sandwich estimator instead of collapsing to a single interaction
  cluster, resolving incorrect standard errors for nested clustering
  designs.
