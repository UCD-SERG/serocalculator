...
# Load posterior curve parameter samples (local example data; no internet required)
curves <- load_sr_params(serocalculator_example("example_curve_params.rds")) |>
  filter(antigen_iso %in% c("HlyE_IgA", "HlyE_IgG"))

# Define the two-phase kinetics function
predicted_ab <- function(t, y0, y1, t1, alpha, r) {
  if (t <= t1) {
    y0 * exp(log(y1 / y0) / t1 * t)
  } else {
    y1 * (1 + (r - 1) * y1^(r - 1) * alpha * (t - t1))^(-1 / (r - 1))
  }
}
predicted_ab <- Vectorize(predicted_ab, vectorize.args = "t")

# Time grid
time_grid <- seq(0, 730, by = 7)

# Loop over all antigen-isotypes
all_summaries <- curves |>
  distinct(antigen_iso) |>
  pull(antigen_iso) |>
  lapply(function(ab) {
    curves_ab <- curves |> filter(antigen_iso == ab)

    pred_matrix <- mapply(
      function(y0, y1, t1, alpha, r) predicted_ab(time_grid, y0, y1, t1, alpha, r),
      y0    = curves_ab$y0,
      y1    = curves_ab$y1,
      t1    = curves_ab$t1,
      alpha = curves_ab$alpha,
      r     = curves_ab$r
    )
    ...
    tibble(
      ...
      var_ab      = apply(pred_matrix, 1, var),
      sd_ab       = apply(pred_matrix, 1, sd)
    )
  }) |>
  bind_rows()

# Plot 1: variance over time, faceted by antigen-isotype, log scale
ggplot(all_summaries, aes(x = time, y = var_ab)) +
  geom_line() +
  scale_y_log10() +
  facet_wrap(~ antigen_iso, scales = "free_y") +
  labs(
    x = "Days since infection",
    y = "Variance of predicted antibody level (log scale)",
    title = "Posterior predictive variance over time since infection"
  ) +
  theme_bw()
```

![](methodology_files/figure-html/unnamed-chunk-9-1.png)
