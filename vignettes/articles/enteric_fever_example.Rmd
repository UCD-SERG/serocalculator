...
For our main findings comparing Bangladesh and Nepal, we should use cluster-robust standard errors to properly account for the geographic clustering in the SEES study:
...
The cluster-robust comparison provides more accurate inference by accounting for within-cluster correlation in the study design.
...
# Extract comparison p-value
comparison_pval <- comparison_clustered$p.value
comparison_pval_formatted <- if_else(
  comparison_pval < 0.001, 
  "< 0.001", 
  paste0("= ", format(round(comparison_pval, 3), nsmall = 3))
)
```

## Conclusions
Using cluster-robust standard errors to account for geographic clustering in the SEES study, we estimate that Bangladesh has significantly higher enteric fever seroconversion rates than Nepal (p `r comparison_pval_formatted`). The overall seroconversion rate in Bangladesh is `r bangla_rate` per 1000 person-years (95% CI: `r bangla_ci_lower`-`r bangla_ci_upper`), compared to `r nepal_rate` per 1000 person-years (95% CI: `r nepal_ci_lower`-`r nepal_ci_upper`) in Nepal. Across age groups, the highest rates are observed among 5- to 15-year-olds in Bangladesh (`r rate_bangla_5_15` per 1000 person-years), which is `r rate_ratio_bangla_nepal` times higher than Nepal in the same age group (`r rate_nepal_5_15` per 1000 person-years). These findings highlight substantial geographic variation in enteric fever transmission, emphasizing the need for targeted prevention strategies.
