...
See the 
[Installation instructions](../../index.html#installing-the-serocalculator-package) for more details.
...
Pathogen-specific sample datasets, noise parameters, and longitudinal antibody dynamics for **serocalculator** are available on the [Serocalculator Data Repository](https://osf.io/ne8pc/) on Open Science Framework (OSF).

This vignette reads local, vendored copies of that data rather than fetching it at render time,
so a documentation build does not depend on OSF being reachable --- see `vignettes/precomputed/osf/README.md` for why and how to refresh them.
Each loading chunk below is followed by the equivalent OSF download,
shown but not evaluated,
so you can see how to pull the same data (or your own) directly into your R environment.
...
To download the full posterior from OSF instead:

```{r curve-osf, eval = FALSE}
curves <-
  "https://osf.io/download/rtw5k/" |>
  load_sr_params()
```

`typhoid_curves_nostrat_100` is that OSF object filtered to `iter %in% 1:100`
(see `data-raw/typhoid_curves_nostrat_100.R`),
so the estimates below average over 100 posterior draws rather than the full
posterior.
That is a coarser Monte Carlo approximation of $\rho(y)$ than the published
v1.4.1 build used,
so the estimates below differ from that build's even where the other two
inputs now match it.
It is one cause among several of the difference from @Aiemjoy_2022_Lancet,
which the note at the top of this vignette already attributes to the analysis
being a simplified version of the published one.
Restoring the full posterior needs a vendored copy of `rtw5k`, which does not
exist yet; see [issue #682](https://github.com/UCD-SERG/serocalculator/issues/682).
...
# Vendored from OSF (osf.io/n6cp3).
# `as_pop_data()`'s defaults already name this file's columns, so no renaming
# arguments are needed: age = "Age", value = "result", id = "index_id".
xs_data <- load_pop_data("../precomputed/osf/n6cp3.rds")
```

To download the same data from OSF instead:

```{r data-osf, eval = FALSE}
xs_data <- load_pop_data("https://osf.io/download/n6cp3/")
```

The package also bundles `sees_pop_data_100`, a much smaller extract of this
same dataset, for examples and tests.
It is a head-slice rather than a random sample
(`dplyr::slice_head()` in `data-raw/sees_pop_data_pakistan_100.R`),
so which subjects it retains is decided by source row order.
That makes it well suited to a fast-running example and unsuitable for
producing a number anyone quotes,
which is why this vignette reads the full data instead.
...
# Vendored from OSF (osf.io/hqy4v).
noise <- load_noise_params("../precomputed/osf/hqy4v.rds")
```

To download the same parameters from OSF instead:

```{r noise-osf, eval = FALSE}
noise <- load_noise_params("https://osf.io/download/hqy4v/")
```

The package's own `example_noise_params_sees` is an earlier snapshot of this
object.
The two are identical except for `y.high`, the upper limit of quantification,
which reads `1000` in the vendored snapshot and `5e+06` in the package data.
`y.high` decides which observations are right-censored in the likelihood,
so the difference would matter if any observation reached either bound.
None does: the largest antibody concentration anywhere in this cross-sectional
dataset is about 219,
so no observation is right-censored under either value and the likelihood is
the same either way.
...
Notice that the point estimates (incidence rates) are identical between the two analyses, but the standard errors are larger with cluster-robust estimation. This reflects the reduced effective sample size due to within-cluster correlation.

The mathematical details of cluster-robust standard errors are explained in the [Cluster-robust standard errors section](../methodology.html#cluster-robust-standard-errors-for-clustered-sampling-designs) of the methodology article.
...
Within each stratum (catchment), the cluster-robust standard errors are larger than the standard errors, appropriately accounting for geographic clustering within each catchment area.
...
```{r cluster_robust_country_comparison}
...
The cluster-robust comparisons provide more accurate inference by accounting for within-cluster correlation in the study design.

```{r include=FALSE}
#Calculate output values for conclusions
...
# Extract cluster-robust results for Bangladesh
bangla_summary <- summary(est_bangladesh_clustered)
bangla_rate <- round(bangla_summary$incidence.rate * 1000, 1)
bangla_ci_lower <- round(bangla_summary$CI.lwr * 1000, 1)
bangla_ci_upper <- round(bangla_summary$CI.upr * 1000, 1)

# Extract cluster-robust results for Nepal
nepal_summary <- summary(est_nepal_clustered)
nepal_rate <- round(nepal_summary$incidence.rate * 1000, 1)
nepal_ci_lower <- round(nepal_summary$CI.lwr * 1000, 1)
nepal_ci_upper <- round(nepal_summary$CI.upr * 1000, 1)

# Extract cluster-robust results for Pakistan
pakistan_summary <- summary(est_pakistan_clustered)
pakistan_rate <- round(pakistan_summary$incidence.rate * 1000, 1)
pakistan_ci_lower <- round(pakistan_summary$CI.lwr * 1000, 1)
pakistan_ci_upper <- round(pakistan_summary$CI.upr * 1000, 1)

# Extract comparison p-values
pval_bangla_nepal <- comparison_bangla_nepal$p.value
pval_bangla_pakistan <- comparison_bangla_pakistan$p.value
pval_nepal_pakistan <- comparison_nepal_pakistan$p.value

# Format p-values
format_pval <- function(p) {
  if_else(p < 0.001, "< 0.001", paste0("= ", format(round(p, 3), nsmall = 3)))
}

pval_bangla_nepal_fmt <- format_pval(pval_bangla_nepal)
pval_bangla_pakistan_fmt <- format_pval(pval_bangla_pakistan)
pval_nepal_pakistan_fmt <- format_pval(pval_nepal_pakistan)
```

## Conclusions
Using cluster-robust standard errors to account for geographic clustering in the SEES study, we observe significant variation in enteric fever seroconversion rates across the three countries. Bangladesh has the highest overall seroconversion rate at `r bangla_rate` per 1000 person-years (95% CI: `r bangla_ci_lower`-`r bangla_ci_upper`), followed by Pakistan at `r pakistan_rate` per 1000 person-years (95% CI: `r pakistan_ci_lower`-`r pakistan_ci_upper`), and Nepal at `r nepal_rate` per 1000 person-years (95% CI: `r nepal_ci_lower`-`r nepal_ci_upper`). Pairwise comparisons show Bangladesh has significantly higher rates than both Nepal (p `r pval_bangla_nepal_fmt`) and Pakistan (p `r pval_bangla_pakistan_fmt`), while the difference between Nepal and Pakistan is also significant (p `r pval_nepal_pakistan_fmt`). 

Across age groups, the highest rates are observed among 5- to 15-year-olds in Bangladesh (`r rate_bangla_5_15` per 1000 person-years), which is `r rate_ratio_bangla_nepal` times higher than Nepal in the same age group (`r rate_nepal_5_15` per 1000 person-years). These findings highlight substantial geographic variation in enteric fever transmission, emphasizing the need for targeted prevention strategies tailored to local epidemiology.
