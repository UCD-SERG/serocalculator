...
## Estimate Seroconversion Rate 
Now we are ready to begin estimating the seroconversion rate. We will conduct two separate analyses using two distinct functions, `estimate_scr()` and `estimate_scr_by()`, to calculate the overall seroconversion rate and the stratified seroconversion rate, respectively.

### Overall Seroconversion Rate
Using the function `estimate_scr()`, we filter to sites in Pakistan and define the datasets for our cross-sectional data (xs_data), longitudinal parameters (curve_param), and noise parameters (noise_param). We also define the antigen-isotype pairs to be included in the estimate (antigen_isos). Here, we have chosen to use two antigen isotypes, but users can add additional pairs if available. 

```{r est}
# Estimate seroconverison rate
...
### Stratified Seroconversion Rates

We can also produce stratified seroconversion rate estimates. Users can select one or more stratification variables in their cross-sectional population dataset. 
Let's compare estimates across all countries and by age group. 

```{r estbycountry}
#Estimate stratified seroconversion rates
...
# Plot seroconversion rate estimates
autoplot(
  est_country_agedf,
  type = "bar",
  yvar = "ageCat",
