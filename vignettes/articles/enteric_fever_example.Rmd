...
This vignette provides users with an example analysis using the **serocalculator** (+HTML LINK HERE) package by reproducing an analysis in @Aiemjoy_2022_Lancet. We review the methods underlying the analysis and then walk through an example of enteric fever incidence in Pakistan. 
...
++Naming conventions
result data needs to be names "Value"
age has to be lower case
...
### ADD: PLOT RESPONSES BY AGE (AGE ON X AXIS, LOG-ANTIBODY RESPONSE ON THE Y AXIS)
Let's take a look at how Ab responses change by age.
```{r}
ggplot(data=pop_data, aes(x=age, y=value, color=Country)) + 
         ...
         theme_bw()
```
how results change by age. If slope is steeper, you have ahigher incidence. highest burden is in banglades . Nepal has a slightly higher incidence in older group (higher slope)
...
est1 = est.incidence(
  ...
  lambda.start=.1,  #.2 per person-year, starting value for incidence estimation where to start the ML 
...
#lets get this working --> not working yet... shows dulicates by 
...
```
(Note that the strata missing warning is not bad -- will just give overall results)

Walk through first examoke on catchment and then on own have them estimate incidence by country (have them write code)-- I will have a separate RMD that is just my notes. 

## ADD plot seroincidence estimates
(Try to get est incidence by country)
```{r}
bar plot with catchment 
...
#rescale incidence rate by 1000
```
