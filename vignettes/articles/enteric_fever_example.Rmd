...
In this scenario, we will limit our analysis to data from Pakistan. We have selected hemolysin E (**HlyE**) as our target antigen and **IgG** and **IgA** as our target immunoglobulin isotypes. Users may select different serologic markers depending on what is available in your data. From the original dataset, we rename our result and age variables to the names required by **serocalculator**. Finally, we once again limit the dataset to only the variables needed for the analysis. 
...
Here, we see that our data is highly skewed with the majority of responses on the lower end of our data, but there is a long tail stretching to over 200. Let's get a better look at the distribution by log transforming our antibody response value. We will also need to update the bin width. 
...
The biological noise, $\nu$ ("nu"), represents error from cross-reactivity to other antibodies. Measurement noise, $\varepsilon$ ("epsilon"), represents error from the laboratory testing process. We also need to rename our lower bound for y. 
