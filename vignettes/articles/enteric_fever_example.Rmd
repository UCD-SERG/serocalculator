...
Comparing `est_multilevel` with `est_with_clustering` above, you'll see that multi-level clustering (accounting for both catchment and cluster correlation) produces larger standard errors than single-level clustering (cluster only), as it accounts for correlation at multiple nested levels.
