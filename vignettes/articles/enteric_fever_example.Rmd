...
ggplot(est_countrydf, aes(y=Country, x=incidence.rate*1000, fill=Country)) +
...
A crucial element of understanding the burden of enteric fever in a population is the age-specific seroincidence. Let's estimate and visualize this by country. 

```{r estbyageCat}
#Using est.incidence.by (strata)

est_age = est.incidence.by(
...
#Save summary(est_age) as a dataframe
est_agedf<- summary(est_age)

#Create barplot (rescale incidence rate and CIs)
ggplot(est_agedf, aes(y=ageCat, x=incidence.rate*1000, fill=ageCat)) +
  facet_wrap(~Country, nrow=3, ncol = 1) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  geom_errorbar(aes(xmin =CI.lwr*1000, xmax=CI.upr*1000, width=.05))+
  labs(
      title= "Enteric Fever Seroincidence by Age",
        x="Seroincience rate per 1000 person-years",
        y="Age Category (years)"
      ) +
  theme_bw() +
  theme(axis.text.y = element_text(size=11),
        axis.text.x = element_text(size=11))
