...
posterior.
That is a coarser Monte Carlo approximation of the marginal
antibody-concentration density $\operatorname{p}(y)$ than the published
v1.4.1 build used,
...
The package's own `example_noise_params_sees` now carries the same values:
`data-raw/example_noise_params_pk.R` builds it by downloading `hqy4v` from OSF,
so either source gives the same noise model.
