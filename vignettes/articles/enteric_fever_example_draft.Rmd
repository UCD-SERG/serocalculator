...
pop_data <- url("https://osf.io/download//n6cp3/") %>% readRDS() %>%
  rename(value = result,
...
noise_param <- url("https://osf.io/download//hqy4v/") %>% readRDS() %>%
...
#  lambda.start=.3,
  ...
  antigen_isos = c("HlyE_IgG", "HlyE_IgA"),
  num_cores = 8, #Allow for parallel processing to decrease runtime 
  build_graph = FALSE
)
...
ggplot(est2df, aes(y=catchment, x=incidence.rate*1000, fill=catchment)) +
  ...
  labs(
      x="Enteric fever seroincience rate per 1000 person-years",
      ) +
  theme_bw() 
