...
# TO DOs: replace with autoplot(age-scatter)
ggplot(data=xs_data, aes(x=eval(parse(text=attributes(xs_data)$age_var)), y=eval(parse(text=attributes(xs_data)$value_var)), color=Country)) + 
  ...
  scale_color_manual(values = c("orange2",'#39558CFF','red'))
