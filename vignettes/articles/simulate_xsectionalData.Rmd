...
2. when `is.na(age_fixed)` then the selected parameter sample is updated for the
age when infection occurs.

There is also a variable `n_mcmc_samples`: when `n_mcmc_samples==0` then a random MC
sample is chosen out of the posterior set (1:4000). When `n_mcmc_samples` is
