...
We need to provide longitudinal antibody decay parameters and noise parameters for the analysis; here, we will manually input them:
...
```{r}
summary(ests) |> select(lambda.sim, cluster, nlm.exit.code)
```

The simulations with `lambda.sim = 0.3` had convergence problems (codes 3-5).

We can also look at graphs of the log-likelihood for each cluster:

```{r, fig.width = 7}
plot(ests)
```

We can see that the log-likelihoods are very flat for the simulations with `lambda.sim = 0.3`; no wonder we had problems!
...
summary(ests) |> select(nlm.exit.code)
```

Now all strata converged.
