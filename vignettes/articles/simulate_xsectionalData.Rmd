...
```{r}
#ggplot(csdata, aes(x=as.factor(antigen_iso), 
#                   y=eval(parse(text=attributes(csdata)$value_var)))) +
...
ggplot(sim.df, aes(x = as.factor(cluster), 
                   y=eval(parse(text=attributes(sim.df)$value_var)))) +
