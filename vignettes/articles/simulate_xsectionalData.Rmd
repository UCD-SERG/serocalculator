...
### The `graph.decay.curves()` function

We can graph individual MCMC samples from the posterior distribution of model parameters using the `graph.decay.curves()` function:
...
dmcmc |> 
  filter(antigen_iso == "HlyE_IgA") |> 
  graph.decay.curves(log_x = TRUE)
```

### The `graph.decay.curves.by()` function

We can simultaneously graph MCMC samples for multiple antigen:isotype combinations using the `graph.decay.curves.by()` function:

```{r}
dmcmc |> graph.decay.curves.by()
```

The `log_x` argument also works here:

```{r}
dmcmc |> graph.decay.curves.by(log_x = TRUE)
```
