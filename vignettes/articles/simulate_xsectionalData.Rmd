...
We can calculate the likelihood of the data, for a given value of the incidence rate (`lambda`):
...
llik(
  ...
  data = csdataL %>%
    rename(
      y = value,
      a = age),
  curve_params = dmcmc |> dplyr::mutate(
...
The argument `verbose = TRUE` prints our progress as we search for 
the value of `lambda` that maximizes the likelihood of the data.
...
We should probably re-run those clusters, adjusting the `nlm()` settings appropriately.
