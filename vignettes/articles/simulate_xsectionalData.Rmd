...
We can visualize the model using
...
summary(ests) |> autoplot(xvar = 'lambda.sim')

```

## graph log(likelihood)

We can graph the log-likelihood of the data as a function of the incidence rate, even without running `est.incidence()`.
