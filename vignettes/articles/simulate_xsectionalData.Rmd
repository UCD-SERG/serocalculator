...
### The `plot_curve_params_one_ab()` function

We can graph individual MCMC samples from the posterior distribution of model parameters using the `plot_curve_params_one_ab()` function:
...
dmcmc |> 
  filter(antigen_iso == "HlyE_IgA") |> 
  plot_curve_params_one_ab(log_x = TRUE)
```

### The `autoplot.curve_params()` function

We can simultaneously graph MCMC samples for multiple antigen:isotype combinations using the `autoplot.curve_params()` function:
