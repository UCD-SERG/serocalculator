...
We can visualize the model using the `graph.curve.params()` function:
...
graph.curve.params(
  curve_params = dmcmc |> 
    ...
    # dplyr::slice_sample(n = 100, by = antigen_iso), # reduce data size for speed
...
library(ggplot2)
summary(ests) |> 
  autoplot(xvar = 'lambda.sim') + ggplot2::geom_abline(ggplot2::aes(intercept = 0, slope = 1))
