...
* 3: decrease the `stepmin` argument to `estimate_scr()`/`est.incidence.by()`
* 4: increase the `iterlim` argument to `estimate_scr()`/`est.incidence.by()`
* 5: increase the `stepmax` argument to `estimate_scr()`/`est.incidence.by()`
