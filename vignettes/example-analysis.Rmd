...
system.file("extdata", "pertussis-iggpt2.rda", package = "serocalculator") |> load() # output: longpar.mc
lp = longpar.mc$IgG
day2yr <- 365.25;
ln.pars <- data.frame(
...
#read simulated data
raw.data = system.file("extdata", "SimulDat.txt", package = "serocalculator") |> read.table(head=TRUE)
...
for(i in 1:length(cat.bnd-1)) 
{
  age.cat[(raw.data$AGE >= cat.bnd[i])
