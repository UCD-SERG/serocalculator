...
The **incidence rate** of a disease for a specific time period is the rate at which individuals in a population are acquiring that disease during that time period. For example, if ten people in a population of 1000 contract typhoid over a one month time period, then the incidence rate for that time period is 10 new cases per 1000 persons per month:
...
More mathematically, the incidence rate *at a given time point*, $t$, is the derivative (i.e., the current rate of change over time) of the cumulative ratio of infections per person.

In both definitions, the units for an incidence rate are "# new infections per # persons at risk per time duration". For convenience, we can rescale the incidence rate to make it easier to understand; for example, we might express incidence as "# new infections per 1000 persons per year" or "# new infections per 100,000 persons per day", etc., depending on the disease in question.
...
Typically, it is difficult to estimate changes from a single time point. However, we can sometimes make assumptions that allow us to do so.

In particular, if we assume that the incidence rate is constant over time, then we can make some progress.

Suppose we measure disease antibody levels $Y$ for a simple random sample of a population at a given calendar time.

For a given member of the sample, let $T$ denote the time since they were last exposed to typhoid. We don't get to observe $T$ - it is a "latent variable". Instead, we only observe $Y$.

If we assume the incidence rate is constant, then the probability that our individual was *last* infected $t$ days ago (i.e., $p(T=t)$) is equal to the probability of being infected at time $t$ (i.e., the incidence rate at time $t$, $\lambda$) times the probability of not being infected after time $t$, which turns out to be $\exp(-\lambda t)$.
...
![Example log(likelihood) curve](https://ucd-serg.github.io/serocalculator/articles/simulate_xsectionalData_files/figure-html/unnamed-chunk-4-1.png){width=80%}

The standard error of the estimate is approximately equal to the inverse of the rate of curvature^[2nd derivative, aka Hessian] in the log-likelihood function, at the maximum (more curvature -> maximum is easier to determine -> less standard error).

## The plot thickens

Unfortunately, we don't observe $T$; we only observe antibody levels $Y$. So things get a little more complicated.
...
Substituting into the previous expression for $p(Y=y)$:
...
Now, the big problem becomes modeling $p(Y=y|T=t)$ (see next section).

## Finding the MLE numerically

A secondary problem is that the likelihood of $Y$ involves the product of integrals, so the log-likelihood involves the sum of the logs of integrals:
