...
Suppose we measure disease antibody levels $Y$ for a simple random sample of a population at a given calendar time. For a given member of the sample, let $\color{red}T$ denote the time since they were last exposed to typhoid. We don't get to observe $T$ - it is a "latent variable". Instead, we only observe $Y$.
...
![Example log(likelihood) curves](https://ucd-serg.github.io/serocalculator/articles/simulate_xsectionalData_files/figure-html/unnamed-chunk-5-1.png){width=100% #fig-loglik}
...
![Example log(likelihood) curves](https://ucd-serg.github.io/serocalculator/articles/simulate_xsectionalData_files/figure-html/unnamed-chunk-5-1.png){}
...
## Antibody response curves: $p(Y=y|T=t)$
