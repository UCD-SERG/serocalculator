...
$$
\hVar{\hat\lambda} \approx
\left[-\llik^{*\prime\prime}(\hat\lambda)\right]^{-1} = \frac{\hat\lambda^2}{n},
\qquad
\hSEf{\hat\lambda} \approx \frac{\hat\lambda}{\sqrt{n}}
$$
...
(@def-likelihood) of the whole sample $\vec{y} = (y_1, y_2, \ldots, y_n)$
factors into the product of their individual contributions
(@prp-marginal-likelihood):

$$
\begin{aligned}
\Lik(\lambda)
&= \prod_{i=1}^n \dens(Y=y_i)
\\&= \prod_{i=1}^n \int_t \dens(Y=y_i|T=t)\,\dens_\lambda(T=t)\,dt\\
\end{aligned}
$$
:::
...
\begin{aligned}
\llik(\lambda)
&= \log \prod_{i=1}^n \int_t \dens(Y=y_i|T=t)\,\dens_\lambda(T=t)\,dt\\
&= \sum_{i=1}^n \log\left\{\int_t \dens(Y=y_i|T=t)\,\dens_\lambda(T=t)\,dt\right\}\\
\end{aligned}
$$
