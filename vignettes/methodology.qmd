...
the incidence rate at a given time point
is the probability of becoming infected at that time point, 
given that you are at risk.
...
## Cross-sectional antibody surveys {.smaller}
...
* In a cross-sectional seroincidence survey, 
we attempt to recruit a random sample (uniform or survey-weighted) 
from the population of interest.

* For each survey participant, we measure antibody levels for the disease of interest ($Y$)

* Each participant has been last been infected at some time duration ($T$)
*prior* to when we measured their antibodies (if they have never been infected, $T$ is infinite).

* We don't get to observe $T$ - it is a "latent variable". 

* Instead, we only observe $Y$.

## Time since infection and incidence

We assume the incidence rate is approximately constant:
...
::: notes
The probability that our individual was **last** infected $t$ days ago, $p(T=t)$, 
