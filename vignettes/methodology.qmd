...
::: {#def-modeling-assumptions}

#### Constant, homogeneous incidence; no lasting immunity

We assume that:

* the incidence rate is approximately **constant over time** and
  **across the population** (**constant and homogeneous incidence**):
  $$\lambda_{i,t} = \lambda, \quad \forall i,t$$
  (subpopulations can be analyzed separately to make this more
  plausible); and

* participants are always at risk of a new infection, regardless of
  how recently they have been infected (**no lasting immunity**).
...
Under those assumptions (Definition @def-modeling-assumptions):

::: {#prp-time-since-infection}

#### Distribution of time since last infection

The time since last infection, $T$, has an **exponential
distribution**, limited by age at observation ($a$): no one can have
been infected before they were born, so the density is supported on
$[0,a]$, and the leftover probability, $\expf{-\lambda a}$, sits on
"never infected":
...
For data and parameter $\lambda$, the **likelihood** $\Lik(\lambda)$ is
the joint probability (or density) of the data, viewed as a function
of the parameter, and the **log-likelihood** is its natural logarithm:
...
The **score function** is the gradient of the log-likelihood with
respect to the (possibly vector-valued) parameter; for the scalar
incidence-rate parameter $\lambda$ used throughout this vignette, this
is just the ordinary derivative:

$$\score(\lambda) \eqdef \llik'(\lambda)$$

A maximum likelihood estimate $\hat\lambda$ satisfies the score
...
log-likelihood with respect to the (possibly vector-valued) parameter;
for the scalar incidence-rate parameter $\lambda$ used throughout this
vignette, this reduces to the ordinary second derivative:
...
The **observed information** is the negative Hessian (Definition
@def-hessian), evaluated at the maximum likelihood estimate:

$$\oinf(\hat\lambda) \eqdef -\hess(\hat\lambda)$$
:::
...
information (Definition @def-observed-information), and the standard
error is the square root of that variance:

$$\hVar{\hat\lambda} \approx \sb{\oinf(\hat\lambda)}^{-1}$$

So more curvature $\rightarrow$
...
For the latent-time exponential model (the fully-observed-$T$ case
from "Likelihood of latent infection times"), the log-likelihood is
$$\llik^*(\lambda) = n\logf{\lambda} - \lambda\sum_{i=1}^n t_i$$

so the score (Definition @def-score) and the Hessian (Definition
@def-hessian; here a scalar second derivative) are
...
so the observed information (Definition @def-observed-information) is
$\oinf(\lambda) = -\llik^{*\prime\prime}(\lambda) = n/\lambda^2$.

Evaluated at the MLE $\hat\lambda = 1/\bar t$ (Proposition
@prp-mle-fully-observed), Theorem @thm-mle-asymptotic-variance gives
...
standard error, consistent with Theorem @thm-mle-asymptotic-variance.
:::
...
$T$ is not purely continuous: recall from Proposition
@prp-time-since-infection that $T$ has a **mixed** distribution ---
continuous on $[0,a]$ with density $\lambda\expf{-\lambda t}$, plus a
discrete atom at $T=\NA$ (never infected) with probability
$\expf{-\lambda a}$. Written out explicitly, the "integral" over $t$
is really an integral over the continuous part plus the
never-infected term:
:::

::: {#prp-marginal-likelihood}

#### Marginal likelihood of an observation

Combining the density of $Y$ given $T$ with the mixed distribution of
$T$ (Proposition @prp-time-since-infection), the marginal likelihood
of a single participant's observed antibody level is
...
never-infected probability from Proposition @prp-time-since-infection.
...
::: {#def-full-sample-likelihood}

#### Product of per-observation likelihoods

The likelihood of the observed data $\vec{y} = (y_1, y_2, \ldots, y_n)$
(Definition @def-likelihood) is the product of each participant's
marginal likelihood (Proposition @prp-marginal-likelihood):
...
::: notes
If we know $p(Y=y|T=t)$, then we can maximize $\Lik(\lambda)$ over $\lambda$ to find the "maximum likelihood estimate" (MLE) of $\lambda$, denoted $\hat\lambda$.
:::
