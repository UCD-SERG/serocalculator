...
not a bug.
Setting `floor_to_standard = TRUE` floors it
at the model-based (non-cluster-robust) variance
$V_{\text{standard}} = H^{-1}$,
the inverse of the observed information at the MLE:

$$V_{\text{final}} = \max\paren{V_{\text{standard}}, V_{\text{multiway}}}$$

With the default `floor_to_standard = FALSE`,
$V_{\text{final}} = V_{\text{multiway}}$ directly,
so a negative inclusion-exclusion sum
still reaches `summary()` as `sqrt()` of a negative number.
