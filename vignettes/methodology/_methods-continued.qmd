...
The seroresponse model is not the problem: this page's two-phase curve
(@eq-rho_y) is the same one the rendered article's own
`methodology/_antibody-response-model.qmd` presents, in the same five
parameters $y_{0}, y_{1}, t_{1}, \alpha, r$.
What differs is the backward-recurrence density, and the fact that the C
engine evaluates only the decaying branch of that curve
(@sec-what-the-engine-computes).
So re-enabling the include would put two derivations of the same kinetics
side by side while leaving the density contradiction in place, which is why
it is better fixed than re-included.
-->
...
@Teunis_2020 (@sec-age-corrected-bwrec).
The difference is not cosmetic: it is a term carrying between about an eighth
and three fifths of the backward-recurrence density at the incidence rates
these data produce (@sec-size-of-the-correction).
:::
...
derives.
It is **not** the density `serocalculator` evaluates, which is
...
That is a different device from the 2012 one
...
The implementation composes @eq-bwrectime_pdf_age with the **decaying branch
alone** of the curve above,
reparameterised as a peak level $A$, a decay rate $k$, and a decay shape $d$.
Those are the same parameters under different names.
`R/f_dev.r` passes `lnpars$y1`, `lnpars$alpha` and `lnpars$d` into the C
entry point `negloglik()`, which forwards them to `NLLFf()`'s `A`, `k` and
`d`, and `R/log_likelihood.R` supplies the last two from a `curve_params`
object as `alpha * 365.25` and `r - 1`.
So

$$
A = y_{1}, \qquad k = \alpha \text{ (per year)}, \qquad d = r-1
$$

and substituting these into $\tau_{-}$ from @eq-rho_y recovers the engine's
own $\tau$ exactly, with the $t_{1}$ offset dropped --- the rising branch
$\rho_{+}$ has no counterpart in `serocalc.c` at all:
...
R(y\vert a) =
\begin{cases}
Q_{a} + P_{a}\left(\text{e}^{-\lambda\tau(y)} - \tau(y)\,
  \dfrac{\text{e}^{-\lambda a}}{a}\right)
  & y_{a}\le y\le A\\[2mm]
Q_{a} & 0\le y < y_{a}\\[1mm]
1 & y > A
\end{cases}
$${#eq-prbF}

The $y > A$ branch is not decoration: `prbF()` sets it explicitly
(`if(A < y) prb_f = 1;`), and the noise models reach it.
`dnsBM()` integrates over ranges bounded by $A+\nu$, so a run with
biological noise evaluates $R$ above $A$ on ordinary data.
...
those are Equations (11), (12), (14) and (15) of the same paper,
and `NLLFf()` itself is Equation (20).
...
So at these rates the omitted term carries between about an eighth and three
fifths of the density,
and between a fifth and three fifths averaged over the sampled ages.
