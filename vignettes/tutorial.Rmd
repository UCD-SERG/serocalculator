...
[ECDC web site](https://www.ecdc.europa.eu/en/publications-data/seroincidence-calculator-tool).
...
^[the corresponding variable names used in the package are listed in monospace font with gray background; see Section \@ref(longParams) for details]

* baseline antibody concentration (`y0`)
* time to peak (`t1`)
...
d) `est.incidence.by`: Main function of the package. Estimates seroincidence based on supplied 
...
Main calculation function provided by package *serocalculator* is called `est.incidence.by`.
...
# Assign output of function "est.incidence.by" to object named 
# "seroincidenceData". Use all available processor cores.
seroincidenceData <- est.incidence.by(
  data = serologyData,
...
The following text should be printed.
    ...
    ## Antibodies   - Input parameter antibodies of function "est.incidence.by".
    ## Strata       - Input parameter strata of function "est.incidence.by".
    ## CensorLimits - Input parameter censorLimits of function "est.incidence.by".
    ## 
...
levels must always be specified when calling function `est.incidence.by` (argument 
